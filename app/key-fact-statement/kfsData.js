// Direct Key Fact Statement static payload extracted from pay10.ae/key-fact-statement/
const kfsData = [
  {
    tabName: "KFS Customer Wallet",
    content: `
				<div class="elementor-element elementor-element-04ae101 elementor-widget elementor-widget-text-editor animated fadeIn" data-id="04ae101" data-element_type="widget" data-e-type="widget" data-settings="{&quot;_animation&quot;:&quot;fadeIn&quot;}" data-widget_type="text-editor.default">
				<div class="elementor-widget-container">
									<h3><strong>KFS – Customer Wallet</strong></h3>
With Pay10 Biz App, you can easily accept and track payments from Pay10 Customers, issue transaction refunds anytime, anywhere, using a mobile device.
<table>
<tbody>
<tr>
<td>Product Name</td>
<td pay10="" biz<="" td="">
</td></tr>
<tr>
<td>Type</td>
<td>Digital Payment Acceptance Solution</td>
</tr>
<tr>
<td>Supported Currency</td>
<td>United Arab Dirhams (AED)</td>
</tr>
<tr>
<td>Eligibility</td>
<td>
<ul>
 	<li>UAE-based Businesses with registered Trade License</li>
 	<li>UAE mobile number</li>
</ul>
</td>
</tr>
<tr>
<td>Key Features</td>
<td>
<ul>
 	<li>Digital onboarding</li>
 	<li>Digital wallet to accept secure online payment.</li>
 	<li>QR code-based payment acceptance (Scan &amp; Pay)</li>
 	<li>Same day settlement</li>
 	<li>Real time balance updates</li>
 	<li>Payment acceptance history</li>
 	<li>Refunds</li>
 	<li>Fund withdrawals</li>
</ul>
</td>
</tr>
<tr>
<td>Required Documents for Account Opening</td>
<td>
<ul>
 	<li>KYC and KYB verification are required.</li>
 	<li>Trade License verification, and Emirates ID verification of Business owner / POA and Shareholders and MOA/AOA to support the company structure.</li>
 	<li>Additional documents may be required as per regulatory guidelines and business type such as:</li>
 	<li>Valid document which has company structure. (Eg: Share certificate/Register of directors/Shareholder</li>
 	<li>Certificate of Incumbency</li>
 	<li>Emirates ID copy of shareholders/POA/UBO)</li>
 	<li>Passport copy of shareholders/POA/UBO)</li>
 	<li>Business address Proof of company</li>
 	<li>POA Documentation</li>
 	<li>Board/Shareholder Resolution</li>
</ul>
</td>
</tr>
<tr>
<td>Fee and Charges</td>
<td>
<ul>
 	<li>Please refer to the Schedule of Charges of our website www.pay10.ae</li>
</ul>
</td>
</tr>
<tr>
<td>Wallet Limits</td>
<td>
<ul>
 	<li>Maximum wallet balance: AED 50,000 – 100,000</li>
 	<li>Daily transaction limit: AED 50,000 – 100,000</li>
 	<li>Monthly transaction limit: AED 50,000 – 100,000</li>
 	<li>Limits may vary based on merchant business type and regulatory requirements</li>
</ul>
</td>
</tr>
<tr>
<td>Security Features</td>
<td>
<ul>
 	<li>Biometric login (fingerprint/face recognition) where supported</li>
 	<li>Login with PIN number</li>
 	<li>Real-time transactions &amp; fraud monitoring</li>
 	<li>Reset password</li>
</ul>
</td>
</tr>
<tr>
<td>Merchant Support</td>
<td>
<ul>
 	<li>24/7 support center: 800-72910</li>
 	<li>Email: <a href="mailto:merchant.support@pay10.ae">merchant.support@pay10.ae</a></li>
 	<li>For more information of our Merchant Payment Solutions, please visit pay10.ae or contact <a href="mailto:sales@pay10.ae">sales@pay10.ae</a></li>
</ul>
</td>
</tr>
<tr>
<td>Complaints and Dispute Resolution</td>
<td>
<ul>
 	<li>Reach out 24/7 support center: 800-72910 or Email: <a href="mailto:merchant.support@pay10.ae">merchant.support@pay10.ae</a> for any Disputes or Complains.</li>
</ul>
</td>
</tr>
</tbody>
</table>
<strong>Important</strong> <strong>Notes</strong>
<ul>
 	<li>Funds in Pay10 Wallet are not interest-bearing</li>
 	<li>Pay10 is licensed and regulated by the Central Bank of the</li>
 	<li>Your consent is required for data sharing with third parties</li>
 	<li>You have the right to withdraw consent for data sharing at any time</li>
 	<li>Pay10 has the right to change the terms and conditions of the product, with a 60-day notice for any</li>
 	<li>This Key Facts Statement provides only indicative fees and charges. For the full and latest fees and charges, please refer to the website for the updated schedule of</li>
 	<li>All fees are exclusive of</li>
</ul>
Wherever applicable, VAT will be

charged at prevailing rates in addition to the fee.
<ul>
 	<li>You can pause, cancel or update the Payment Initiation Permissions granted through Altareq (Open Banking) at any</li>
 	<li>Pay10 disclaims liability for losses arising from unauthorized transactions caused by your failure to safeguard your</li>
</ul>
<strong>Warning</strong>

By using the Pay10 Digital Wallet, you acknowledge that:
<ol>
 	<li>Your account may be suspended if you fail to provide the required documentation or if the information provided is inaccurate or</li>
 	<li>Pay10 may share your information with regulatory authorities in compliance with applicable laws and</li>
 	<li>You are responsible for the conﬁdentiality of your account information and for all transactions made through your account, you must keep your Pay10 wallet credentials safe and conﬁdential at all</li>
 	<li>If you see a transaction on your account that you do not recognize, or you think may be incorrect, you must contact us immediately via the call center or our website.</li>
 	<li>You acknowledge that Pay10 will not be liable for any losses due to unauthorized use of your wallet if such unauthorized use results from your failure to keep credentials conﬁdential</li>
</ol>
For full details, please refer to our Terms and Conditions available at <a href="http://www.pay10.ae/terms">http://www.pay10.ae/terms</a>

Please note that this KFS will be updated from time to time as per mandate from Central Bank.								</div>
				</div>
				`
  },
  {
    tabName: "KFS Biz UAE App",
    content: `
					<div class="e-con-inner">
				<div class="elementor-element elementor-element-5782ea8 elementor-widget elementor-widget-text-editor" data-id="5782ea8" data-element_type="widget" data-e-type="widget" data-widget_type="text-editor.default">
				<div class="elementor-widget-container">
									<h3><strong>KFS Bizz App</strong></h3>
With Pay10 Biz App, you can easily accept and track payments from Pay10 Customers, issue transaction refunds anytime, anywhere, using a mobile device.
<table>
<tbody>
<tr>
<td>Product Name</td>
<td>Pay10 Biz</td>
</tr>
<tr>
<td>Type</td>
<td>Digital Payment Acceptance Solution</td>
</tr>
<tr>
<td>Supported Currency</td>
<td>United Arab Dirhams (AED)</td>
</tr>
<tr>
<td>Eligibility</td>
<td>
<ul>
 	<li>UAE-based Businesses with registered Trade License</li>
 	<li>UAE mobile number</li>
</ul>
</td>
</tr>
<tr>
<td>Key Features</td>
<td>
<ul>
 	<li>Digital onboarding</li>
 	<li>Digital wallet to accept secure online payment.</li>
 	<li>QR code-based payment acceptance (Scan &amp; Pay)</li>
 	<li>Same day settlement</li>
 	<li>Real time balance updates</li>
 	<li>Payment acceptance history</li>
 	<li>Refunds</li>
 	<li>Fund withdrawals</li>
</ul>
</td>
</tr>
<tr>
<td>Required Documents for Account Opening</td>
<td>
<ul>
 	<li>KYC and KYB verification are required.</li>
 	<li>Trade License verification, and Emirates ID verification of Business owner / POA and Shareholders and MOA/AOA to support the company structure.</li>
 	<li>Additional documents may be required as per regulatory guidelines and business type such as:</li>
 	<li>Valid document which has company structure. (Eg: Share certificate/Register of directors/Shareholder</li>
 	<li>Certificate of Incumbency</li>
 	<li>Emirates ID copy of shareholders/POA/UBO)</li>
 	<li>Passport copy of shareholders/POA/UBO)</li>
 	<li>Business address Proof of company</li>
 	<li>POA Documentation</li>
 	<li>Board/Shareholder Resolution</li>
</ul>
</td>
</tr>
<tr>
<td>Fee and Charges</td>
<td>
<ul>
 	<li>Please refer to the Schedule of Charges of our website www.pay10.ae</li>
</ul>
</td>
</tr>
<tr>
<td>Wallet Limits</td>
<td>
<ul>
 	<li>Maximum wallet balance: AED 50,000 – 100,000</li>
 	<li>Daily transaction limit: AED 50,000 – 100,000</li>
 	<li>Monthly transaction limit: AED 50,000 – 100,000</li>
 	<li>Limits may vary based on merchant business type and regulatory requirements</li>
</ul>
</td>
</tr>
<tr>
<td>Security Features</td>
<td>
<ul>
 	<li>Biometric login (fingerprint/face recognition) where supported</li>
 	<li>Login with PIN number</li>
 	<li>Real-time transactions &amp; fraud monitoring</li>
 	<li>Reset password</li>
</ul>
</td>
</tr>
<tr>
<td>Merchant Support</td>
<td>
<ul>
 	<li>24/7 support center: 800-72910</li>
 	<li>Email: <a href="mailto:merchant.support@pay10.ae">merchant.support@pay10.ae</a></li>
 	<li>For more information of our Merchant Payment Solutions, please visit pay10.ae or contact <a href="mailto:sales@pay10.ae">sales@pay10.ae</a></li>
</ul>
</td>
</tr>
<tr>
<td>Complaints and Dispute Resolution</td>
<td>
<ul>
 	<li>Reach out 24/7 support center: 800-72910 or Email: <a href="mailto:merchant.support@pay10.ae">merchant.support@pay10.ae</a> for any Disputes or Complains.</li>
</ul>
</td>
</tr>
</tbody>
</table>
<strong>Important Notes</strong>
<ul>
 	<li>Funds in Pay10 Wallet are not interest-bearing</li>
</ul>
<ul>
 	<li>Pay10 is regulated by the Central Bank of UAE</li>
</ul>
<ul>
 	<li>Your consent is required for data sharing with third parties</li>
</ul>
<ul>
 	<li>You have the right to withdraw consent for data sharing at any time</li>
 	<li>Pay10 has the right to change the terms and conditions of the product, with a 60-day notice for any changes in fees or</li>
 	<li>This Key Facts Statement provides only indicative fees and charges. For the full and latest fees and charges, please refer to the website for the updated schedule of</li>
 	<li>All fees are exclusive of Wherever</li>
</ul>
applicable; VAT will be charged at the prevailing rates in addition to the fee.

<strong>Warning</strong>

By using the Pay10 BIZ App, you acknowledge that:
<ol>
 	<li>Your account may be suspended if you fail to provide the required documentation or if the information provided is inaccurate or</li>
 	<li>Pay10 may share your information with regulatory authorities in compliance with applicable laws and Biz App credentials safe and conﬁdential.</li>
 	<li>You are responsible for the conﬁdentiality of your account information and for all actions conducted through your account, you must always ensure your Pay10 Biz App credentials safe and conﬁdential.</li>
 	<li>If you see a transaction/withdrawal or Refund on your account that you do not recognize, or you think may be incorrect, you must contact us immediately via the call center or our website.</li>
 	<li>Refunds for the full transaction value are available through the Pay10 Biz App to assist merchants in processing returns or addressing customer claims. Pay10 bears no liability for any misuse of this feature by business employees or authorized users of the Pay10 Biz App.</li>
</ol>
For full details, please refer to our Terms and Conditions available at <a href="http://www.pay10.ae/terms-services-2">http://www.pay10.ae/terms-service</a>

Please note that this KFS will be updated from time to time as per mandate from Central Bank.

&nbsp;								</div>
				</div>
					</div>
				`
  },
];

export default kfsData;
